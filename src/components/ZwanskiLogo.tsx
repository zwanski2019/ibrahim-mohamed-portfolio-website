...
const ZwanskiLogo: React.FC<ZwanskiLogoProps> = ({
  ...
  return <div className={`flex items-center cursor-pointer transition-all duration-200 hover:scale-105 focus:scale-105 focus:outline-none focus:ring-2 focus:ring-primary/50 rounded-lg p-1 ${className}`} onClick={onClick} onKeyDown={e => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      onClick?.();
    }
  }} tabIndex={0} role="button" aria-label="Navigate to home page">
      {/* Logo icon/symbol */}
